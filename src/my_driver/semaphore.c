...
static void
iree_hal_strela_semaphore_destroy(iree_async_semaphore_t *semaphore) {
  printf("%s\n", __func__);
}

static uint64_t
iree_hal_strela_semaphore_query(iree_async_semaphore_t *base_semaphore) {
  iree_hal_strela_semaphore_t* semaphore = (iree_hal_strela_semaphore_t*)base_semaphore;
  return iree_atomic_load(&semaphore->payload_value, iree_memory_order_acquire);
}

static iree_status_t
iree_hal_strela_semaphore_signal(
  ...
  // iree_hal_strela_hw_signal_semaphore(..., value);
...
static void
iree_hal_strela_semaphore_on_fail(
...
iree_hal_strela_semaphore_vtable = {
  .async = {
    .destroy = iree_hal_strela_semaphore_destroy,
    .query = iree_hal_strela_semaphore_query,
    .signal = iree_hal_strela_semaphore_signal,
    .on_fail = iree_hal_strela_semaphore_on_fail,
  },
