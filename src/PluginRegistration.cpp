...
struct MySession : public PluginSession<MySession, MyOptions> {
  ...
  extendInputConversionPreprocessingPassPipeline(
    ...
    passManager.addNestedPass<func::FuncOp>(strela::createNormalizeRescalePass());
  }
  ...
  extendCustomInputConversionPassPipeline(
    OpPassManager& passManager, std::string_view typeMnemonic
      ...
      if (options.enable_fusion) {
        passManager.addNestedPass<func::FuncOp>(strela::createFuseConv2DPass());
  ...
  extendPreprocessingPassPipeline(OpPassManager &passManager) override {
    if (options.partition) {
      passManager.addPass(strela::createFormStrelaDispatchesPass());
    }
  }
