...
using mlir::estela::centered_matmul_bitstream;
...
struct Conv2DOffload : public OpRewritePattern<linalg::Conv2DNhwcHwcfQOp> {
  ...
  LogicalResult matchAndRewrite(
...

    {
      ...
      auto kernel = centered_matmul_bitstream;
      ...
      auto kernelTensorType = RankedTensorType::get(
        {static_cast<int64_t>(centered_matmul_bitstream.size())}, i32
      );
      auto denseAttr = DenseElementsAttr::get(kernelTensorType, ArrayRef(centered_matmul_bitstream));
...
struct MyFusionPass : public PassWrapper<MyFusionPass, OperationPass<func::FuncOp>> {
  ...
  StringRef getArgument() const override { return "iree-strela-fuse-conv2d"; }
